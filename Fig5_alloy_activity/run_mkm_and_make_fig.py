...
#pot_state = {'CO': 'pot', 'OCCO': 'pot'} #Use the result at q=0 ('q0') or at the potential ('pot')
if len(sys.argv) < 2:
...
setup_file = '../fig_tools/mkm_file.mkm'
...
#alloy_CO_data = json.load(open(f'../data/alloy_CO_binding_results.json','r'))
all_data['vacuum']['CO']['E_bind_min'].update(json.load(open(f'../data/CO_binding_results.json','r'))[0])
all_data['lattices'] = json.load(open(f'../data/CO_binding_results.json','r'))[-1]

mkm_out_pklfile=f'mkm_results_{potential:1.2f}V_{facet}.pkl'
...
add_scaling_line(ax,all_data,facet,pot_state,free_en_corr,plot_line=False)
plot_Cu_points(ax,all_data,pot_state,free_en_corr, facets=['100','111','211'], plot_line=True)
add_alloy_points(ax,all_data, pot_state, free_en_corr, really_interesting_comps,tms=['Ag','Au','Cu','Pd','Pt','Rh','Ni','Ir'])

ax.set_ylim([-2.1,0.9]) #,np.array(phs).max()])
#ax.set_xlim([np.array(pots).min(),np.array(pots).max()])
ax.set_xlim([np.array(pots).min(),0.4])
